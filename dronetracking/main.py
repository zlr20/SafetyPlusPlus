...
# python main.py --task cartpole --overrides config_overrides/traj_task.yaml
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", type=str)
    parser.add_argument("--overrides",nargs='+',type=str)
    parser.add_argument("--exp_name",type=str)
    parser.add_argument("--env", default="SafetyCartPole-v0")      # Env name
    ...
    parser.add_argument("--kappa",default = 1)                      # Penalized factor for Safety++
    ...
    parser.add_argument("--lam_lr",default = 1e-4)                # Step-size of multiplier update
    ...
    assert [bool(i) for i in [args.use_td3,args.use_usl,args.use_recovery,args.use_qpsl,args.use_lag,args.use_fac]].count(True) == 1, 'Only one option can be True'
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            if args.use_usl:
                evalEpRet,evalEpCost = eval_policy(policy, eval_env, args.seed,use_usl=True)
            elif args.use_recovery:
                evalEpRet,evalEpCost = eval_policy(policy, eval_env, args.seed,use_recovery=True)
            elif args.use_qpsl:
                evalEpRet,evalEpCost = eval_policy(policy, eval_env, args.seed,use_qpsl=True)
            else:
                evalEpRet,evalEpCost = eval_policy(policy, eval_env, args.seed)
